...
export const Button = ({text, type, click, icon, disabled, className}: Props) => {
  ...
  return (
    <button disabled={disabled}  onClick={click} className={`w-full  text-white items-center justify-center bg-yellow-600  h-7 px-3 rounded-md hover:bg-yellow-700 cursor-pointer ${className}`}>{text}</button>
  )
}
